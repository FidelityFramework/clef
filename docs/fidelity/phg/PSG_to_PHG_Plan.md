...
**I1 — Enumerated source sets.** Each derived relationship retains its finite, ordered participants, roles and multiplicity. At a closure site this supplies the direct slot-obligation frontier, as specified by closure representation §11. Referenced storage has separate lifetime, ownership and sharing premises; its transitive reachability is not bounded by counting fields. Unresolved premises remain explicit. A relation is not a proof because its inputs exist, and convergence/decidability require the owning analysis's sound transfer and convergence arguments. See the [closure settlement contract](../../../../Composer/docs/Closure_Settlement_Contract.md).
