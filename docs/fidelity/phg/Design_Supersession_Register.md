...
- **Diagnostic numbering.** The spec's access-kinds chapter numbers the ReadOnly-store diagnostic CCS8020; clef's `ccs-specification.md` Appendix D still carries the FS8xxx series (FS8002 for the same message). The chapters cite CCS8020. The two series should be reconciled; not done here.
...
- **Inline-by-default retired as a stated principle.** Both NTU documents (`spec/native-type-universe.md` §6.3, `clef/docs/fidelity/native-type-universe.md`) presented the absorbed library's inline-by-default model as Clef's; it was tried and reverted because it exploded the PSG. Rewritten to the explicit-`inline` policy of the attributes chapter (mandatory for SRTP generalization and for lifting a scope-bounded buffer, discouraged elsewhere, the optimizer deciding the rest). The gate retires the phrase.
