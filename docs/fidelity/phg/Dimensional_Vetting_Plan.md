...
| L-13 | `Project/ProjectChecker.fs` `buildPlatformContext` | the width dimensions are derived from one `WordSize` (`Some 64` or `Some 32`) and otherwise fall back to the x86_64 defaults; the platform description declares no dimension by name and no representation | silent default of a platform fact | CS-7b (D8): the descriptor declares its dimensions and representations by name; an absent declaration is a diagnostic, never x86_64 |
