    ...
    | SynPat.LongIdent(SynLongIdent(idents, _, _), _, _, argPats, _, _) ->
        ...
        | _ ->
            let payloadTypes, tagIndex =
                    ...
                    let constructorType =
                        ...
                        | NativeType.TForall(parameters, body) ->
                            let arguments = parameters |> List.map (fun tp ->
                                freshTypeParamAuto tp.Kind range |> typeParameterArgument)
                ...
                | None ->
                    addError pat.Range $"The constructor '{caseName}' is not defined." env
                    [], 0
            ...
            | SynArgPats.Pats pats ->
                ...
                if pats.Length <> payloadTypes.Length then
                    addError pat.Range $"Constructor '{caseName}' expects {payloadTypes.Length} fields, got {pats.Length}" env
                let patterns, bindings =
            ...
            | SynArgPats.NamePatPairs _ ->
                addError pat.Range "Named constructor field patterns are not supported" env
