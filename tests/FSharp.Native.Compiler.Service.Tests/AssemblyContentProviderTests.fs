module FSharp.Native.Compiler.Service.Tests.AssemblyContentProviderTests
...
let private checkFile (source: string) = 
    let _, checkFileAnswer = 
        checker.ParseAndCheckFileInProject(filePath, 0, FSharp.Native.Compiler.Text.SourceText.ofString source, projectOptions) 
        |> Async.RunImmediate
