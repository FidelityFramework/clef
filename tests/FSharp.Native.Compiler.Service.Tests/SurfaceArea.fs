...
namespace FSharp.Native.Compiler.Service.Tests.SurfaceArea
...
type SurfaceAreaTest() =
    ...
    member _.VerifySurfaceAreaFSharpCompilerService() =
        ...
        let assembly =
            let path = Path.Combine(Path.GetDirectoryName(typeof<int list>.Assembly.Location), "FSharp.Native.Compiler.Service.dll")
            Assembly.LoadFrom path

        let baseline = Path.Combine(__SOURCE_DIRECTORY__, $"FSharp.Native.Compiler.Service.SurfaceArea.{platform}.bsl")
        let outFileName = Path.Combine(__SOURCE_DIRECTORY__, $"FSharp.Native.Compiler.Service.SurfaceArea.{platform}.out")
